...
In traditional rule based programming you write exact rules yourself (if the message contains “refund” then label = refund).
In supervised machine learning you show the computer many examples with the correct answers and it learns the patterns by itself.
3. In the support-intent dataset, what are the example, feature, label and prediction?
Example = one full row (one support message + its information)
...
Classification chooses one category from a fixed list (refund, technical_issue…).
Regression predicts a number (like price or temperature).
Here we need a category, not a number.
5. Why is it supervised learning?
Because every training example already has the correct label. The model learns by comparing its guesses to the real answers.
6. Why should test data not be used during training or daily tuning?
If you use the test data while building the model, the model can “cheat” and the final score will look better than it really is. The test set must stay hidden until the very end.
7. What is the role of the validation set?
It is the practice test. We use it to try different ideas and choose the best model settings without touching the real final test set.
8. What is a baseline, and why did we build one before logistic regression?
A baseline is a very simple model (here it always guesses the most common class).
...
If 95% of messages are “normal”, a model that always says “normal” gets 95% accuracy but completely fails on the important rare cases.
...
High precision means support agents do not waste time on wrong tickets.
...
Macro F1 gives equal importance to every class, even the rare ones. Accuracy can hide bad performance on small classes.
...
Training score is very high (almost perfect) but validation score is much lower. The model memorised the training data instead of learning general rules.
18. What is data leakage? Give two examples.
Data leakage is when the model sees information it should not have.
Examples:
...
So you know exactly what caused the change in the score. If you change many things at once, you cannot tell which change helped or hurt.
...
The model is underfitting (or the task is too hard with the current features). It did not learn useful patterns.
...
how exactly TF-IDF calculates the numbers
